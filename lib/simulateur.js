...
(function(window) {
	...
	function makeModule() {
		...
		style.href = "http://ethernety.free.fr/extension4scratch/ScratchX/lib/style.css";
		...
		addEvent(version, 'click', function() { window.open('http://ethernety.net'); });
		// }}}
			...
			return {
				...
				'tone': function() {
					if(audio==null) {
						audio = newAudio();
						if(audio!=null) {
							audio.src = "http://ethernety.free.fr/extension4scratch/ScratchX/lib/buzzer.mp3";
							audio.loop = true;
						}
					}
					if(audio!=null)
		...
		function newTSmatrixLED(w, h) {
			...
			for(var y=0; y<h; y++) {
				...
				for(var i=0; i<w; i++) {
					...
					light[y*w+i] = d.addSVG('circle');
					fillSVG(light[y*w+i], {'fill':'#000', 'opacity': '0', 'stroke': 'none', 'stroke-width':'0', 'cx':(bx+x*10+5), 'cy':(by+y*10+5), 'r':'4'});
				}
		...
		function newTSneoLED() {
			...
			var g = {
				...
				'sendRequest': function(mode, req) {
					if(matrices['L'+req[0]]!=undefined) {
						switch(mode) {
						case 'fixeLED':
							matrices['L'+req[0]].set(req);
							break;
						case 'resetLED':
							matrices['L'+req[0]].reinit();
							break;
						}
					} else {
						error("err_2", {port:req[0]});
					}
				},
				'serialize': function() {
					var serMat = '';
			...
			try {
				...
				case 'initLED': // TSNeoLed matrix LED
					if(priv.TSNeoLed==null) {
						...
						priv.TSNeoLed.addMatrix(msg[0], msg[1], msg[2]);
					}
					break;
				...
				case 'resetLED': // TSNeoLed effacer la matrice
					if(priv.TSNeoLed==null) {
						...
						priv.TSNeoLed.sendRequest(order, msg);
					}
					break;
				// }}}
				}
