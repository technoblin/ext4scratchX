...
(function(window) {
	...
	function makeModule() {
		...
		function newGroveServo() {
			...
			var rot = d.addSVG('g');
			newRotorOnce(rot);
			fillSVG(rot, {'transform':'translate(10,0) rotate(90)'});
			return {
				'isServo': true,
				'element': d,
				'connector': con,
		...
		function newRotorOnce(g) {
			var plain = newSVG('path');
			fillSVG(plain, {'fill': '#333', 'stroke':'#333', 'stroke-width':'0', 'd':'m3,-3 h4 v2 c-1.5,0 -1.5,2 0,2 c1.5,0 1.5,-2 0,-2 v-2 h3 v2 c-1.5,0 -1.5,2 0,2 c1.5,0 1.5,-2 0,-2 v-2 h3 v2 c-1.5,0 -1.5,2 0,2 c1.5,0 1.5,-2 0,-2 v-2 c6,0 6,6 0,6 h-13 c-6,0 -6,-6 0,-6 z'});
			g.appendChild(plain);
			var border = newSVG('path');
		...
		function newGrovePi() {
			...
			return {
				...
				'addModule': function(name, option) {
					...
					case 'Grove Servo':
						if(cons['D'+option.addr]!=undefined)
							normalDeclaration(name, ['D'+option.addr], function() { return newGroveServo(); });
						break;
					}
				},
			...
			try {
				...
				case 'setDigit':
					...
					case '11': // IR Line Finder
						...
						break;
					case '12': // Servo motor
						addGrovePiModule('Grove Servo', {addr:addr});
						break;
					}
					break; // }}}
