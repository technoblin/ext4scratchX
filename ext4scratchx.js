...
new (function() {
	...
	// Ensemble des outils {{{
	var ext_tools = {};
	ext_tools.newRoll = function(max) {
		var priv = {r:0,w:0,0:0};
		return {
			push: function(v) {
				if(priv[priv.w]!=v) {
					priv.w = (priv.w+1)%max;
	...
	ext_tools.newStack = function(type, max) {
		if(type==undefined) {
			var priv = newRoll(max);
			return {
				push: function(v) {
					priv.push(v);
				},
		...
		for(var i=1; i<type.length; i++)
			priv[type[i]] = newRoll(max);
		return {
			push: function(v) {
				v = v.split(':');
				if(v[0]==type[0])
					for(var i=1; i<type.length; i++)
						priv[type[i]].push(v[i]);
			},
			pop: function(t, v) {
				if(t==type[0] && priv[v]!=undefined)
					return priv[v].pop();
				return undefined;
			},
		};
	...
	ext_tools.getSensorData = function(boardID, addr, type, val) {
		var idx = this.getIndexSensor(boardID, addr);
		if(idx == -1)
			return 0;
		return this.sensorDataArray[idx].stack.pop(type, val);
	};
	ext_tools.keyGen = function(boardID, addr) {
		if(this.isSimulateBoard(boardID))
			return 'S'+boardID+addr;
		else if(this.isLinkedBoard(boardID, true))
			return 'R'+this.boardLinkedArray[boardID]+addr;
		else
			return -1;
	};
	ext_tools.setIndexSensor = function(boardID, addr, type) {
		var kg = this.keyGen(boardID, addr);
		for(var i=0; i<this.sensorDataArray.length; i++)
			if(this.sensorDataArray[i].key === keyGen)
				break;
		this.sensorDataArray[i] = {'key': keyGen, 'stack': this.newStack(type, 10)};
		return i;
	};
		...
		if(mc>=11 && !this.isVersion(boardID, "20180322")) {
			...
			if(!Number.isNaN(mc) && !Number.isNaN(addr)) {
				if(this.isDigitSensor(mc)) {
					var sensorId = this.setIndexSensor(boardID, 'D'+addr, this.getDigitSensorType(mc));
					opt = sensorId+'/'+opt;
				}
	...
	ext_tools.setAnalogPin = function(boardID, addr, module, opt) {
		...
		if(!Number.isNaN(mc) && !Number.isNaN(addr)) {
			if(this.isAnalogSensor(mc)) {
				var sensorId = this.setIndexSensor(boardID, 'A'+addr, this.getAnalogSensorType(mc));
				opt = sensorId+'/'+opt;
			}
	...
	ext_tools.setModuleMode = function(boardID, module, opt) {
		...
		if(!Number.isNaN(mc)) {
			if(this.isModuleSensor(mc)) {
				var sensorId = this.setIndexSensor(boardID, this.getModuleAddr(mc), this.getModuleSensorType(mc));
				opt = sensorId+'/'+opt;
			}
	...
	ext_tools.getDigitSensorType = function(n) {
		var list = {9: 'DHT:1:2'};
		return list[n];
	};
	...
	ext_tools.getAnalogSensorType = function(n) {
		var list = {4: 'JOY:1:2:3'};
		return list[n];
	};
	ext_tools.isModuleSensor = function(n) {
		return this.getModuleAddr(n)!=undefined;
	};
	...
	ext_tools.getModuleSensorType = function(n) {
		var list = {5: 'GPS:1:2:3:-', 7: 'MTB:4:5:6'};
		return list[n];
	};
	...
	ext.getDigitValue = function(boardID, addr, cmd) {
		...
		case 1:
		case 2:
			return ext_tools.getSensorData(boardID, addr, 'DHT', cmd);
		case 3:
			return ext_tools.getSensorData(boardID, addr);
		}
		return 0;
	...
	ext.getAnalogValue = function(boardID, addr, cmd) {
		...
		case 2:
		case 3:
			return ext_tools.getSensorData(boardID, addr, 'JOY', cmd);
		}
		return 0;
	};
	...
	ext.getModValue = function(boardID, cmd) {
		...
		case 3: // GPS nombre de satelites
			return ext_tools.getSensorData(boardID, 'SP', 'GPS', cmd);
		case 4: // miniTrackBall X
		case 5: // miniTrackBall Y
		case 6: // miniTrackBall Confirmation
			return ext_tools.getSensorData(boardID, 'i0x4a', 'MTB', cmd);
		}
		return 0;
	};
	...
	var descriptor = {
		blocks: [
			...
			['w', "X0 - Carte %m.bdNum Adresse IP: %s", 'setBoard', '1', 'virtual'],
...
*/
			...
			// Gestion des matrices de LEDs
			['r', "X9.1 - Calculer RVB %n %n %n", 'calcRVB', '255', '255', '255'],
			...
			['r', "G9.5.1 - Carte %m.bdNum Lire sur %m.digitPin la valeur %m.digitValue", 'getDigitValue', '1', 'Choisir une E/S', 'Choisir une valeur'],
			['r', "G9.5.2 - Carte %m.bdNum Lire sur %m.analogPin la valeur %m.analogValue", 'getAnalogValue', '1', 'Choisir une E/S', '1. Joystick X'],
			// Utilitaires
		...
		menus: {
			...
			modValue: ['1. GPS Latitude', '2. GPS Longitude', '3. GPS Nb satelites', '4. TrackBall X', '5. TrackBall Y', '6. TrackBall Confirmation'],
